 // ========== 开始修改：添加音频预加载系统 ==========
// 全局音频预加载状态
const audioPreloadSystem = {
    preloaded: false,
    audioInstance: null,
    
    // 预加载音频
    preloadAudio: function(soundFile) {
        
        console.log('预加载音频');
        if (this.preloaded) return;
        
        try {
            this.audioInstance = new Audio(soundFile);
            this.audioInstance.volume = 0.01; // 很小音量
            this.audioInstance.muted = true; // 静音
            this.audioInstance.preload = 'auto';
            
            console.log('预加载音频: ',this.audioInstance);
            
            // 尝试播放以触发预加载
            this.audioInstance.play().then(() => {
                console.log('音频预加载试play成功');
                this.audioInstance.pause();
                this.preloaded = true;
            }).catch(e => {
                console.log('音频预加载试play失败，等待用户交互:', e);
                // 即使失败也标记为已尝试预加载
                this.preloaded = true;
                // 设置超时，确保不会阻塞
               setTimeout(() => { 
                    console.log('第二次音频预加载试play失败，等待用户交互:', e);
            ...
            // 设置超时，确保不会阻塞
            setTimeout(() => {
                if (this.audioInstance && !this.preloaded) {
                    this.preloaded = true;
                } 
            }, 1000);
        } catch (error) {
            console.log('音频预加载系统初始化失败:', error);
            this.preloaded = true;
        }
    },
    
    // 获取新的音频实例（避免多个实例同时播放）
    getAudioInstance: function(soundFile) {
        // 确保已预加载
        if (!this.preloaded) {
            this.preloadAudio(soundFile);
        }
        
        // 返回新的音频实例
        try {
            const audio = new Audio(soundFile);
            audio.volume = 0.3; // 正常音量
            audio.loop = true;
            return audio;
        } catch (error) {
...
function typeWriter(selector, text, options = {}) {
    const config = {
        ...
        splitChar: '',
        // ========== 开始修改：添加音效选项 ==========
        sound: false, // 是否播放打字音效
        soundFile: '../music/traditional-typer.mp3', // 音效文件路径
    ...
    if (config.sound) {
        // 确保音频已预加载
        audioPreloadSystem.preloadAudio(config.soundFile);
        
        // 获取音频实例
        typeSound = audioPreloadSystem.getAudioInstance(config.soundFile);
        
        if (typeSound) {
            typeSound.loop = true;
            typeSound.volume = config.soundVolume;
            
            // 随机起始时间
            typeSound.addEventListener('loadedmetadata', function() {
                if (typeSound && typeSound.duration) {
                    const maxStartTime = typeSound.duration * 0.7;
                    const minStartTime = typeSound.duration * 0.3;
                    soundStartTime = Math.random() * (maxStartTime - minStartTime) + minStartTime;
                }
            }, { once: true });
        }
    }
    ...
    const delayTimer = setTimeout(() => {
        ...
        if (typeSound) {
            ...
            const playSound = () => {
                if (soundStartTime > 0) {
                    typeSound.currentTime = soundStartTime;
                }
                
                typeSound.play().catch(e => {
                    // 静默处理播放失败（浏览器自动播放限制）
                    console.log('音效播放失败（浏览器限制），等待用户交互:', e.message);
                });
            };
...
typeWriter.stop = function(selector) {
    ...
    if (element && element._typewriterTaskId) {
        // ========== 开始修改：停止音效 ==========
        if (element._typewriterConfig && element._typewriterConfig.sound) {
            // 如果有音效配置，停止相关音效
            const soundElements = document.querySelectorAll('audio');
            soundElements.forEach(audio => {
                if (audio.src.includes('traditional-typer.mp3')) {
                    audio.pause();
